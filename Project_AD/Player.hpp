...
public:
	...
	sf::FloatRect getRect() {//ô-öèÿ ïîëó÷åíèÿ ïðÿìîóãîëüíèêà. åãî êîîðä,ðàçìåðû (øèð,âûñîò).
		return sf::FloatRect(x, y, w, h);//ýòà ô-öèÿ íóæíà äëÿ ïðîâåðêè ñòîëêíîâåíèé 
	}
...
private:
	...
	void checkCollisionWithMap(float Dx, float Dy)
	{
		...
		for (int i = 0; i < objects.size(); i++)//ïðîõîäèìñÿ ïî îáúåêòàì
			if (getRect().intersects(objects[i].rect))//ïðîâåðÿåì ïåðåñå÷åíèå èãðîêà ñ îáúåêòîì
			{
				if (objects[i].name == "solid")//åñëè âñòðåòèëè ïðåïÿòñòâèå
				{
...
public:
	...
	Bullet(sf::Image& image, sf::String Name, TileMap * lvl, float X, float Y, int W, int H, int dir) :Entity(image, Name, X, Y, W, H) {//âñ¸ òàê æå, òîëüêî âçÿëè â êîíöå ñîñòîÿíèå èãðîêà (int dir)
