...
{
public:
	inline Object(float x, float y, float width, float height) : rect(x, y, width, height)
	{

	}

	inline int         GetPropertyInt(const std::string& name);
	inline  float       GetPropertyFloat(const std::string& name);
	inline std::string GetPropertyString(const std::string& name);
...
public:
	...
	inline 	bool load(const std::string& tmx_file_path);

	inline  Object               getObject(const std::string& name);
	inline  std::vector<Object>  getObjectsByName(const std::string& name);
	inline  std::vector<Object>  getObjectsByType(const std::string& type);
